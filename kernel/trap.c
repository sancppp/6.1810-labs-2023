...
void usertrap(void) {
  ...
  if (r_scause() == 8) {
    ...
    // lab10 -> hit.2&5 lazy 处理用户态传来的缺页中断
    // 分配一个新物理页，读文件中的4096B到这个页中
    // 1. 找到引发缺页中断的虚拟地址所对应的vma
    struct VMA* vma = 0;
    uint64 va = r_stval();
    ...
    // 2.获取一个新的物理页
    uint64 new_page = 0;
    if ((new_page = (uint64)kalloc()) == 0) goto failed;
    memset((void*)new_page, 0, PGSIZE);
    // 3.初始化PTE权限 PROT_* -> PTE_*
    int flag = PTE_U;
    if (vma->prot & PROT_READ) flag |= PTE_R;
    if (vma->prot & PROT_WRITE) flag |= PTE_W;
    if (vma->prot & PROT_EXEC) flag |= PTE_X;
    // 4.建立映射
    if (mappages(p->pagetable, va, PGSIZE, new_page, flag) == -1) {
      kfree((void*)new_page);
      goto failed;
    }
    // 4.将文件([va+offset,va+offset+4096))内容写进内存，hit.5
    struct inode* inode = vma->file->ip;
    ilock(inode);
    // 坑，这里的offset是inode文件的offset指针
    readi(inode, 1, va, va - vma->addr, PGSIZE);
    iunlock(inode);
    // for (int i = 0; i < PGSIZE; i++) {
    //   printf("(%d,%x) ", i, ((char*)new_page)[i]);
    // }
