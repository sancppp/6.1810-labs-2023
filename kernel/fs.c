...
static uint bmap(struct inode *ip, uint bn) {
  ...
  // 前11块，返回直接块地址
  if (bn < NDIRECT) {
    if ((addr = ip->addrs[bn]) == 0) {  // 如果指针为0就分配一个新的
      addr = balloc(ip->dev);
      if (addr == 0) return 0;
      ip->addrs[bn] = addr;
    }
  ...
  // bn-NDIRECT < 256，返回间接引用
  if (bn < NINDIRECT) {
    // Load indirect block, allocating if necessary.
    if ((addr = ip->addrs[NDIRECT]) == 0) {  // 如果索引没有就分配一个
      addr = balloc(ip->dev);
      if (addr == 0) return 0;
  ...
  if (bn < NINDIRECT * NINDIRECT) {
    uint address[3];  // [0] -> [1] -> [2] -> data

    //[0]
    if ((address[0] = ip->addrs[NDIRECT + 1]) == 0) {  // 如果索引没有就分配一个
      address[0] = balloc(ip->dev);
      if (address[0] == 0) return 0;
      ip->addrs[NDIRECT + 1] = address[0];
    }
    //[0] -> [1]
    bp = bread(ip->dev, address[0]);
    a = (uint *)bp->data;  // 强转成uint的数组
    if ((address[1] = a[bn / NINDIRECT]) == 0) {
      address[1] = balloc(ip->dev);
      if (address[1]) {
        a[bn / NINDIRECT] = address[1];
        log_write(bp);
      }
    }
    brelse(bp);
    //[1] -> [2]
    bp = bread(ip->dev, address[1]);
    a = (uint *)bp->data;  // 强转成uint的数组
    if ((address[2] = a[bn % NINDIRECT]) == 0) {
      address[2] = balloc(ip->dev);
      if (address[2]) {
        a[bn % NINDIRECT] = address[2];
        log_write(bp);
      }
    }
    brelse(bp);
