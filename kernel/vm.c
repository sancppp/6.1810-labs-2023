...
// 默认逻辑是将old页表里指向的物理地址的内存内容全部复制给new
// 现在需要改为将old页表直接复制一份给new，并且将权限改为只读
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz) {
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  for (i = 0; i < sz; i += PGSIZE) {
    // walk(pg,va,alloc):在pg中找到va对应的pte，如果alloc为1&&没找到则申请内存页
    if ((pte = walk(old, i, 0)) == 0) panic("uvmcopy: pte should exist");
    // 判断pte是否合法
    if ((*pte & PTE_V) == 0) panic("uvmcopy: page not present");
    if (PTE_FLAGS(*pte) & PTE_W) {  // 不可写的页不用COW
      *pte &= ~PTE_W;               // 将父进程拥有的pte的PTE_W位置零
      *pte |= PTE_COW;              // 设置为COW
    }
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
...
int uvmcopy_backup(pagetable_t old, pagetable_t new, uint64 sz) {
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    if (*pte & PTE_COW) {
      ...
      // 直接更新PTE
      *pte = (PTE_FLAGS(*pte) & ~PTE_COW) | PTE_W | PA2PTE(new_pa);
    }

    if (pte == 0 || (*pte & PTE_V) == 0 || (*pte & PTE_U) == 0 ||
        (*pte & PTE_W) == 0)  // 这里判断了PTE_W位，所以COW操作要在这之前！
      return -1;
    pa0 = PTE2PA(*pte);
    n = PGSIZE - (dstva - va0);
    if (n > len) n = len;
