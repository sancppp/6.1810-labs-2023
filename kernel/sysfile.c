...
uint64 sys_mmap(void) {
  ...
  argint(2, &prot);
  argint(3, &flags);
  if (argfd(4, &fd, &file) < 0) goto failed;
  argint(5, &offset);

  if (addr < 0 || length < 0 || prot < 0 || offset < 0) goto failed;
  if ((prot & PROT_READ) && !file->readable) goto failed;
  if ((flags & MAP_SHARED) && (prot & PROT_WRITE) && !file->writable)
    goto failed;

  // 在进程内存地址空间中找一个空闲的，建立映射，找一个空闲的vma进行记录。
  // 难点，应该分配哪里的空闲地址？+内存对齐
  // p->sz字段是堆内存的上界，具体看sys_sbrk的实现。只要p->sz小于VMMAX，就说明还有空闲的地址空间
  struct proc *p = myproc();
  if (p->sz + length > MAXVA) goto failed;
  ...
  vma->file = file;
  vma->free_len = 0;
  // hit.4 mmap的实现需要增加file的引用计数
  filedup(file);
#ifdef DEBUG
  printf("sys_mmap: addr = %x.\n", addr);
#endif
  return vma->addr;

failed:
  return 0xffffffffffffffff;  // 其实等价于-1
}

// Implement munmap: find the VMA for the address range and unmap the specified
// pages (hint: use uvmunmap). If munmap removes all pages of a previous mmap,
// it should decrement the reference count of the corresponding struct file. If
// an unmapped page has been modified and the file is mapped MAP_SHARED, write
// the page back to the file. Look at filewrite for inspiration.
...
uint64 sys_munmap(void) {
  ...
  argaddr(0, &addr);
  argint(1, &length);
  if (addr < 0 || length < 0) goto failed;
  ...
  if (vma == 0) goto failed;
  // 写回文件
  if (vma->flags & MAP_SHARED) filewrite(vma->file, addr, length);

  uvmunmap(p->pagetable, addr, PGROUNDDOWN(length) / PGSIZE, 1);
  // 有可能一次申请，多次释放
  if (PGROUNDUP(length) >= vma->length) {
